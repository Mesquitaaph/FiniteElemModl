...
function montaLGSerial(ne, base)
    LG = zeros(Int64, base, ne)

    for i in 1:ne
        LG1 = (base-1)*i - (base-2)
        for a in 1:base
            @inbounds LG[a,i] = LG1 + (a-1)
        end
    end
...
function montaEQSerial(ne, neq, base)
    EQ = zeros(Int64, (base-1)*ne+1, 1)
...
function montaKSerial(base, ne, neq, dx, alpha, beta, EQoLG::Matrix{Int64})
    npg = base; P, W = legendre(npg)
...
function solveSys(base, alpha, beta, ne, a, b, f, u)
    ...
    EQ = montaEQSerial(ne, neq, base); LG = montaLGSerial(ne, base)
    EQoLG = EQ[LG]
...
println("Rodando \n")
# let
#     alpha, beta, a, b, u, u_x, f = examples(2); ne = 2^15; base = 2
